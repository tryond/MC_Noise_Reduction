...
// Ray cam(Vec(50, 52, 200.0), Vec(0, -0.042612, -1).normalize());

Ray camStart(Vec(50, 52, 295.6), Vec(0, -0.042612, -1).normalize());
Ray camEnd(Vec(50, 52, 200.0), Vec(0, -0.042612, -1).normalize());
int frames = 1;
...
int main(int argc, char *argv[]) {
    ...
    Vec camOrig = camEnd.o - camStart.o;
    camOrig = camOrig / frames;

    Vec camDir = camEnd.d - camStart.d;
    camDir = camDir / frames;

    Ray camMove(camOrig,camDir);
    ...
    int dFoc = argc >= 14 ? atoi(argv[13]) : -1;
    int gMean = argc >= 15 ? atoi(argv[14]) : 0;
    int gFoc = argc >= 16 ? atoi(argv[15]) : -1;
...
#pragma omp critical
    ...
    c = dofGray(c,o,d,h,w,gMean,gFoc);      // arg 13, arg 14
